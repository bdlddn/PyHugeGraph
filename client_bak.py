...
class HugeGraphClient(HugeGraphBase):
    ...
    def __init__(self, host, graph_name):
        super().__init__(host, graph_name)
    ...
    def scheme(self):
        if self._scheme:
            return self._scheme
        self._scheme = SchemaManager(self._host, self._graph_name)
        return self._scheme

    def graph(self):
        if self._graph:
            return self._graph
        self._graph = GraphManager(self._host, self._graph_name)
        return self._graph
    ...
    def get_graph_config(self):
        ...
        :return:
        """
        url = self._host + "/graphs" + "/" + self.graph + "conf?token=" + self.token
        response = requests.get(url, headers=self.headers)
        res = Response(response.status_code, response.content)
        return res
